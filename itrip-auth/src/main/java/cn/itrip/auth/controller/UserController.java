...
public class UserController {
    ...
    public Dto registerByPhone(@RequestBody ItripUserVO vo) {
        ...
        try {
            ...
            userService.creatUserByPhone(user);
    ...
    @ApiOperation(value = "通过手机号注册", httpMethod = "POST", response = Dto.class, notes = "手机号注册")
    ...
    public Dto registerByMail(@RequestBody ItripUserVO userVO) {
        if (!validMail(userVO.getUserCode())) {
        ...
        try {
            ...
            userService.creatUserByMail(user);
            return DtoUtil.returnSuccess("注册成功");
        } catch (Exception e) {
            e.printStackTrace();
            return DtoUtil.returnFail(e.getMessage(), ErrorCode.AUTH_UNKNOWN);
